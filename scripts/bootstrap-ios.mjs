#!/usr/bin/env node
/**
 * After `npx cap add ios`, wire the custom NativeHaptics plugin into the Xcode app target:
 * - copy Swift sources
 * - set Main.storyboard customClass = BridgeViewController
 * - ensure both .swift files are in PBX Sources / FileRef / App group
 *
 * Usage:
...
const FILES = [
  {
    name: 'NativeHapticsPlugin.swift',
...
function patchStoryboard() {
  ...
  // Normalize any CAPBridgeViewController / broken dual customModule
  xml = xml.replace(
  ...
  // Collapse accidental duplicate customModule attributes
  xml = xml.replace(
  ...
  if (!xml.includes('customClass="BridgeViewController"')) {
    console.warn('Could not patch Main.storyboard automatically — set customClass to BridgeViewController in Xcode.');
    return;
  }
...
function ensureGroupChild(pbx, f) {
  ...
  // Insert after AppDelegate.swift in App group children
...
function patchPbxproj() {
  ...
  // Heal known corruption from older bootstrap versions (broken AppDelegate build file line)
  pbx = pbx.replace(
...
function main() {
  ...
  console.log('  3) Select your iPhone → Run');
  console.log('  4) Change appId in capacitor.config.json before shipping');
}
